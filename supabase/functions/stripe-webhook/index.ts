...
serve(async (req) => {
  try {
    ...
    if (kind === 'fiscalisation') {
      ...
      // Extends hosting_expires_at -- App.jsx gates access for a Standard/
      // Pro tenant whose hosting has lapsed (new-signup tenants only, see
      // 1786090000_hosting_fee.sql).
