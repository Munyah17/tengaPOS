-- Production bug: a vendor could have real receipt details saved under one
-- specific branch while the tenant-wide default (branch_id IS NULL) stayed
-- completely blank. Any session/print that resolved to the default scope
-- (no branch pinned, e.g. the owner viewing "all branches") got a blank
-- receipt even though the tenant *had* configured their details somewhere.
-- fetchEffectiveReceiptConfig (client) now prefers whichever row is actually
-- populated, but this closes the gap at the source too: whenever a vendor
-- saves a branch-scoped config and no tenant-wide default row exists yet,
-- also seed the default with the same values so it's never left blank.
...
BEGIN
  ...
  -- Seed the tenant-wide default so it's never left blank once a vendor has
  -- configured *something* — only for the vendor's own approved saves
  -- (shop-manager drafts shouldn't overwrite/create the tenant default).
