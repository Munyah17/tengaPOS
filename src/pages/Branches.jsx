...
export default function Branches() {
  ...
  const branches = isDemo
    ? DEMO_BRANCHES
    : profile?.branch
      ? [{ id: 1, name: profile.branch.name || 'Main Branch', location: '', staff: 0, revenue: 0, status: 'active' }]
      : []
    ...
    <div className="p-6">
      ...
      {branches.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 py-20 dark:border-slate-700">
          ...
          <p className="text-sm font-medium text-slate-500">No branches configured</p>
          <p className="mt-1 text-xs text-slate-400">Contact support to set up your branch locations</p>
        </div>
      ) : (
