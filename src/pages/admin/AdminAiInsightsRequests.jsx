...
import { Sparkles, CheckCircle, XCircle, Clock, Banknote, Zap } from 'lucide-react'
...
export default function AdminAiInsightsRequests() {
  ...
  const load = async () => {
    const [{ data: reqs }, { data: tenantData }] = await Promise.all([
      supabase.from('ai_insights_requests').select('*, tenants(name, slug, features)').order('created_at', { ascending: false }).limit(100),
  ...
  const activate = async (tenantId, months, tenantName) => {
    ...
    const { data: updated, error } = await supabase
      .from('tenants')
      .update({ features: { ...(t?.features || {}), ai_insights: true }, ai_insights_expires_at: expires.toISOString() })
      .eq('id', tenantId)
      .select('id')
  ...
  const decide = async (req, approve) => {
    ...
    try {
      ...
      await supabase
        ...
        .eq('id', req.id)
      toast.success(approve ? `AI Insights unlocked for ${req.tenants?.name}` : 'Request rejected')
      load()
    } catch (err) {
  ...
  const manualAssign = async () => {
    ...
    try {
      ...
      toast.success(`AI Insights manually activated for ${t?.name}`)
      load()
    } catch (err) {
      toast.error(err.message)
    } finally {
  ...
  const suspend = async (tenantId, name) => {
    ...
    try {
      ...
      await supabase
        .from('tenants')
        .update({ features: { ...(t?.features || {}), ai_insights: false }, ai_insights_expires_at: null })
        .eq('id', tenantId)
      await supabase.from('audit_logs').insert({
        actor_id: user?.id,
        actor_email: user?.email,
        action: 'ai_insights_suspended',
        target_type: 'tenant',
        target_id: tenantId,
        details: { tenant_name: name },
      })
      toast.success(`AI Insights suspended for ${name}`)
      load()
    } catch (err) {
      toast.error(err.message)
    } finally {
  ...
  const active = tenants.filter((t) => t.features?.ai_insights === true)

  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">AI Insights Requests</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Cash payment approvals + manual activation of the AI Insights add-on ($1/month)
        </p>
      </div>
          ...
          >
            ...
            {tenants.map((t) => (
              <option key={t.id} value={t.id}>
                {t.name}{t.features?.ai_insights ? ' (already active)' : ''}
              </option>
            ))}
          </select>
          <select
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-white/5">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="font-bold text-slate-900 dark:text-white">Active AI Insights ({active.length})</h2>
        </div>
        {active.length === 0 ? (
