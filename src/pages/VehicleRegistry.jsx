          ...
          {filteredCustomers.map((c) => (
            <div key={c.id} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                {c.phone && <span className="flex items-center gap-1 text-xs text-slate-500"><Phone className="h-3 w-3 flex-shrink-0" />{c.phone}</span>}
              </div>
              {(c.vehicles || []).length === 0 ? (
                <p className="ml-0 text-xs text-slate-400 sm:ml-6">No vehicles on record</p>
              ) : (
                <div className="ml-0 space-y-1.5 sm:ml-6">
                  {c.vehicles.map((v) => (
                    <button
                      ...
                      className="flex w-full flex-wrap items-center justify-between gap-1.5 rounded-xl border border-slate-100 px-3 py-2 text-left text-sm hover:border-amber-300 hover:bg-amber-50/50 dark:border-slate-800 dark:hover:border-amber-700 dark:hover:bg-amber-950/20"
                    >
                      <span className="flex min-w-0 flex-wrap items-center gap-2 text-slate-700 dark:text-slate-300">
                        <Car className="h-3.5 w-3.5 flex-shrink-0 text-slate-400" />
                        <span className="truncate">{[v.make, v.model, v.year].filter(Boolean).join(' ') || 'Vehicle'}</span>
                        {v.reg_number && <span className="flex-shrink-0 rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs dark:bg-slate-800">{v.reg_number}</span>}
                      </span>
